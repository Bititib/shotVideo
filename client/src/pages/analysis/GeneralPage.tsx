...
export default function GeneralPage() {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full">
      {/* Input Panel */}
      <section className="earth-input-panel w-full lg:w-[400px] shrink-0 h-fit lg:h-full lg:overflow-y-auto border-r border-white/5 bg-black [&::-webkit-scrollbar]:hidden" style={{ scrollbarWidth: 'none' }} aria-labelledby="general-analysis-title">
        <div className="p-6 flex flex-col min-h-full">
          <h2 id="general-analysis-title" className="text-xl font-semibold text-white mb-1 flex items-center gap-3">
            <PlaySquare className="w-6 h-6 text-zinc-300" /> 开始通用分析
          </h2>
          <p className="text-xs text-zinc-500 mb-6 pl-9">从一段影像出发，沉淀可复用的创意洞察</p>

          {/* URL Input */}
          <div className="mb-4">
            <label className="block text-xs font-semibold text-zinc-400 mb-2 uppercase tracking-wider">视频链接导入</label>
            <div className="flex gap-2">
              <input type="url" value={urlInput} onChange={e => setUrlInput(e.target.value)} placeholder="粘贴 TikTok/YouTube/直链" className="flex-1 min-w-0 bg-white/[0.03] border-none rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:ring-1 focus:ring-white/20 placeholder:text-zinc-600" aria-label="视频链接" />
          ...
          <div className="earth-upload-zone mb-4 border border-dashed border-white/10 rounded-2xl overflow-hidden h-[210px] transition-colors hover:border-blue-500/30 cursor-pointer flex items-center justify-center"
            ...
            onClick={() => !previewUrl && fileRef.current?.click()} role="button" tabIndex={previewUrl ? -1 : 0} aria-label="上传视频文件"
            onKeyDown={e => { if (!previewUrl && (e.key === 'Enter' || e.key === ' ')) { e.preventDefault(); fileRef.current?.click(); } }}>
            {previewUrl ? (
      ...
      {/* Result Panel */}
      <section className="earth-result-panel flex-1 overflow-y-auto p-6 lg:p-8 [&::-webkit-scrollbar]:hidden" style={{ scrollbarWidth: 'none' }} aria-label="分析结果">
        {isAnalyzing ? <LoadingOverlay /> : result ? (
          ...
          <div className="flex-1 flex items-center justify-center h-full min-h-[360px]">
            <div className="earth-empty-state flex flex-col items-center justify-center text-center">
              <div className="w-14 h-14 rounded-2xl bg-white/[0.05] border border-white/10 flex items-center justify-center mb-5" aria-hidden="true">
                <PlaySquare className="w-7 h-7 text-blue-400" />
              </div>
              <h3 className="text-base font-semibold text-white mb-2">等待一段值得深挖的影像</h3>
              <p className="text-zinc-500 text-sm max-w-xs">上传视频或导入链接，AI 将从内容结构、镜头语言与传播策略中提炼创意线索。</p>
            </div>
          </div>
        )}
      </section>
